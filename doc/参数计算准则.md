...
- 若用于训练与报告，代码中会将其统一换算为：

$$N_{pl} (kN) = \frac{A_s f_y + A_c f_c}{1000}$$
...
为了支持新的 `eta_u / r / Npl` 主线、样本级分层和 regime 诊断，当前代码会由 `scripts/compute_feature_parameters.py` 离线生成以下派生列；其中在输入含 `Nexp (kN)` 时也会把 `eta_u` 和 `r` 一并持久化输出：
...
$$r = \eta_u - 1 = \frac{N_{exp} - N_{pl}}{N_{pl}}$$
...
当前规则优先基于原始数据中的 `Group` 列判定：`A -> square/rectangular`（再按 `b == h` 区分）、`B -> circular`、`C -> obround`；仅在缺少 `Group` 时回退到几何关系判定。该列主要用于 regime analysis 和后续分组实验。
